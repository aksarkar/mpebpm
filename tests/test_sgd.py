...
def test_ebpm_gamma_batch(simulate_gamma):
  ...
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat = mpebpm.sgd.ebpm_gamma(x, s, batch_size=n, max_epochs=2000)
...
def test_ebpm_gamma_minibatch(simulate_gamma):
  x, s, log_mu, log_phi, l0 = simulate_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat = mpebpm.sgd.ebpm_gamma(x, s, batch_size=100, max_epochs=500, lr=1e-2)
  l1 = -st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat))).logpmf(x).sum()
  assert np.isfinite(log_mu_hat).all()
  assert np.isfinite(neg_log_phi_hat).all()
  assert log_mu_hat.shape == (1, p)
...
def test_ebpm_gamma_minibatch_shuffle(simulate_gamma):
  x, s, log_mu, log_phi, l0 = simulate_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat = mpebpm.sgd.ebpm_gamma(x, s, batch_size=100, max_epochs=500, lr=1e-2, shuffle=True)
  l1 = -st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat))).logpmf(x).sum()
  assert np.isfinite(log_mu_hat).all()
  assert np.isfinite(neg_log_phi_hat).all()
  assert log_mu_hat.shape == (1, p)
...
def test_ebpm_gamma_sgd(simulate_gamma):
  ...
  log_mu_hat, neg_log_phi_hat = mpebpm.sgd.ebpm_gamma(x, s, batch_size=1, max_epochs=10, lr=5e-3)
  l1 = -st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat))).logpmf(x).sum()
  assert np.isfinite(log_mu_hat).all()
  assert np.isfinite(neg_log_phi_hat).all()
  assert log_mu_hat.shape == (1, p)
...
def test_ebpm_point_gamma_oracle_init(simulate_point_gamma):
  ...
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(x, s, init=(log_mu, -log_phi), batch_size=n, max_epochs=2000)
...
def test_ebpm_point_gamma_batch(simulate_point_gamma):
  x, s, log_mu, log_phi, logodds, l0 = simulate_point_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(x, s, batch_size=n, max_epochs=2000)
  pi0hat = sp.expit(logodds_hat)
  Fhat = st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat)))
  llik_nonzero = np.log(1 - pi0hat) + Fhat.logpmf(x)
  l1 = -np.where(x < 1, np.log(pi0hat + np.exp(llik_nonzero)), llik_nonzero).sum()
...
def test_ebpm_point_gamma_minibatch(simulate_point_gamma):
  x, s, log_mu, log_phi, logodds, l0 = simulate_point_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(x, s, batch_size=100, max_epochs=500)
  pi0hat = sp.expit(logodds_hat)
  Fhat = st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat)))
  llik_nonzero = np.log(1 - pi0hat) + Fhat.logpmf(x)
  l1 = -np.where(x < 1, np.log(pi0hat + np.exp(llik_nonzero)), llik_nonzero).sum()
...
def test_ebpm_point_gamma_minibatch_shuffle(simulate_point_gamma):
  x, s, log_mu, log_phi, logodds, l0 = simulate_point_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(x, s, batch_size=100, max_epochs=500, shuffle=True)
  pi0hat = sp.expit(logodds_hat)
  Fhat = st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat)))
  llik_nonzero = np.log(1 - pi0hat) + Fhat.logpmf(x)
  l1 = -np.where(x < 1, np.log(pi0hat + np.exp(llik_nonzero)), llik_nonzero).sum()
...
def test_ebpm_point_gamma_sparse(simulate_point_gamma):
  ...
  y = ss.csr_matrix(x)
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(y, s, batch_size=100, max_epochs=500)
  pi0hat = sp.expit(logodds_hat)
  Fhat = st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat)))
  llik_nonzero = np.log(1 - pi0hat) + Fhat.logpmf(x)
  l1 = -np.where(x < 1, np.log(pi0hat + np.exp(llik_nonzero)), llik_nonzero).sum()
...
def test_ebpm_point_gamma_sgd(simulate_point_gamma):
  x, s, log_mu, log_phi, logodds, l0 = simulate_point_gamma
  n, p = x.shape
  log_mu_hat, neg_log_phi_hat, logodds_hat = mpebpm.sgd.ebpm_point_gamma(x, s, batch_size=1, max_epochs=30, lr=5e-3)
  pi0hat = sp.expit(logodds_hat)
  Fhat = st.nbinom(n=np.exp(neg_log_phi_hat), p=1 / (1 + s * np.exp(log_mu_hat - neg_log_phi_hat)))
  llik_nonzero = np.log(1 - pi0hat) + Fhat.logpmf(x)
  l1 = -np.where(x < 1, np.log(pi0hat + np.exp(llik_nonzero)), llik_nonzero).sum()
